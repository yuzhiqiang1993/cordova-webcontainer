...
public class Geolocation extends CordovaPlugin {
    ...
    public boolean execute(String action, JSONArray args, CallbackContext callbackContext) throws JSONException {
        LOG.d(TAG, "进入 execute 方法, action=" + action);
        context = callbackContext;
        if (action.equals("getPermission")) {
            boolean highAccuracy = args.getBoolean(0);
            LOG.d(TAG, "正在处理 getPermission 请求, highAccuracy=" + highAccuracy);
    ...
    public void onRequestPermissionResult(int requestCode, String[] permissions, int[] grantResults) throws JSONException {
        LOG.d(TAG, "触发 onRequestPermissionResult 权限请求回调, requestCode=" + requestCode);
        PluginResult result;
        ...
        if (context != null) {
            ...
            LOG.d(TAG, "用户已同意定位权限，返回 OK 状态给前端");
            result = new PluginResult(PluginResult.Status.OK);
            context.sendPluginResult(result);
        } else {
            LOG.d(TAG, "context 为空，无法回调前端");
        }
    }
    ...
    public boolean hasPermisssion(String[] permissions) {
        for (String p : permissions) {
            if (!PermissionHelper.hasPermission(this, p)) {
                LOG.d(TAG, "检查权限发现缺失: " + p);
                return false;
            }
        }
        LOG.d(TAG, "检查权限完成: 所有需要的权限均已存在");
        return true;
    }
